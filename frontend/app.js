...
// Preview Image Selection (UI-05, UI-06)[cite: 1]
fileInput.addEventListener('change', (e) => {
  const file = e.target.files[0];
  if (!file) return;

  // Reset existing polling if picking a new file (POLL-06)[cite: 1]
  stopPolling();
...
// Primary Upload Submission Handler (SUB-01, SUB-02)[cite: 1]
processBtn.addEventListener('click', async () => {
  const { isSubmitting, selectedFile } = getState();
  if (isSubmitting || !selectedFile) return;

  const requestStart = performance.now();
  setState({ isSubmitting: true, observationError: false });

  try {
    const data = await DataService.uploadImage(selectedFile);
    const ackLatency = performance.now() - requestStart; // Measure Ack Latency[cite: 1]
    ...
    startPolling(data.status_url); // POLL-01[cite: 1]
  } catch (err) {
...
// Polling Control Loop (POLL-02 to POLL-10)[cite: 1]
function startPolling(statusUrl) {
  stopPolling();

  const controller = new AbortController();
  const timer = setInterval(() => poll(statusUrl, controller.signal), 1000); // ~1 sec interval[cite: 1]
...
function stopPolling() {
  ...
  if (abortController) abortController.abort(); // Cancel active GETs (POLL-06)[cite: 1]
...
async function poll(statusUrl, signal) {
  try {
    ...
    if (jobData.status === 'completed' || jobData.status === 'failed') {
      stopPolling(); // POLL-05[cite: 1]
      setState({ activeJob: jobData, variants: jobData.variants || [] });
    } else {
      setState({ activeJob: jobData }); // queued or processing[cite: 1]
    }
  } catch (err) {
    if (err.name === 'AbortError') return;

    // Retrieval Error Policy: Stop polling, preserve job, show "Try again" (POLL-07 to POLL-10)[cite: 1]
    stopPolling();
    setState({ observationError: true });
  }
}

// Try Again Handler (POLL-09, POLL-10)[cite: 1]
